  ...
  async getSignUrl(channelId) {
    ...
    for (const item of body.data.rows) {
      ...
      if (playUrl.indexOf('?') > 0) {
        const realUrl = playUrl.substring(playUrl.indexOf(',') + 1)
        let resp = await axios.get(realUrl)
        const realBody = resp.data
        return new Promise(resolve => {
          resolve(realBody.substring(realBody.indexOf('http')))
        })
      }
    }
  }
